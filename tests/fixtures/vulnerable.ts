...
function badQuery(userId: string) {
  const sql = // SORK TODO: SQL_INJECTION - configure an AI provider (sork config use-preset ...) for an auto-fix, or fix manually
'SELECT * FROM users WHERE id = ' + userId; // SQL_INJECTION
  return sql;
}
