...
export async function reviewFile(filePath: string): Promise<ReviewResult> {
  ...
  const verdict: 'APPROVE' | 'WARN' | 'BLOCK' =
    criticals > 0 ? 'BLOCK' : highs > 0 || aiArtifacts > 0 ? 'WARN' : 'APPROVE';

  const summary =
    verdict === 'APPROVE'
      ? `no critical issues  ·  ${rel}`
      : verdict === 'WARN'
        ? `${highs} high${aiArtifacts > 0 ? `, ${aiArtifacts} ai artifacts` : ''}  ·  review before merging`
...
function verdictLine(r: ReviewResult): void {
  ...
  console.log('  ' + col(tag) + '  ' + c.white(r.file) + c.faint(`  [${r.language}]`));
...
export async function reviewStagedFiles(): Promise<void> {
  ...
  console.log(
    c.faint('  │') +
      c.teal('  SORK Review') +
      c.faint(`  ·  ${files.length} staged file(s)`) +
      c.faint(' '.repeat(Math.max(0, 34 - String(files.length).length)) + '│')
  );
  console.log(c.faint('  ╰' + '─'.repeat(58) + '╯'));
  console.log('');
    ...
    try {
      ...
      if (result.aiArtifacts > 0) {
        console.log(
          '  ' +
            c.faint(' '.repeat(9)) +
            c.purple(`${result.aiArtifacts} ai-generated pattern(s)  ·  verify before merging`)
        );
      }
