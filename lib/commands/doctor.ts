...
export async function runDoctor(projectPath: string): Promise<void> {
  ...
  const report: HealthReport = {
    name, totalFiles: 0, languages: {}, criticalFiles: [],
    aiArtifacts: 0, tornCode: 0, nullCrashes: 0,
    secretsFound: 0, errorSwallow: 0, totalIssues: 0, score: 100,
  };
  ...
  for (const file of files) {
    ...
    try {
      ...
      report.aiArtifacts  += stability.filter(i => (i as {aiGenerated?:boolean}).aiGenerated).length;
      report.tornCode     += stability.filter(i => (i as {category?:string}).category === 'torn_code').length;
      report.nullCrashes  += stability.filter(i => (i as {category?:string}).category === 'null_crash').length;
      report.secretsFound += vulns.filter(i => (i as {category?:string}).category === 'secrets').length;
      report.errorSwallow += stability.filter(i => (i as {category?:string}).category === 'error_swallow').length;
  ...
  const deductions =
    report.secretsFound * 20 + report.tornCode * 10 +
    report.nullCrashes  * 6  + report.errorSwallow * 4 +
    report.aiArtifacts  * 3  + Math.min(report.totalIssues * 0.4, 25);
  ...
  for (const [lang, count] of sorted) {
    console.log(
      ...
      '  ' + c.dim(String(count).padStart(3)) + c.faint(' files')
    );
  }
  ...
  console.log(
    ...
    '  ' + scoreLabel(report.score) + c.dim(' / 100') +
  ...
  const findings = [
    { key: 'secrets',       count: report.secretsFound, color: c.red,    note: 'rotate credentials immediately'                 },
    { key: 'torn code',     count: report.tornCode,     color: c.amber,  note: 'unimplemented stubs will crash in production'   },
    { key: 'ai artifacts',  count: report.aiArtifacts,  color: c.purple, note: 'verify generated logic is production-ready'     },
    { key: 'null crashes',  count: report.nullCrashes,  color: c.yellow, note: 'missing null guards on chained calls'           },
    { key: 'error swallow', count: report.errorSwallow, color: c.amber,  note: 'empty catch blocks hiding failures'             },
  ];
  ...
  if (report.secretsFound > 0) {
    console.log('  ' + c.red(pad('urgent', 10)) + '  ' + c.red(`${report.secretsFound} credential(s) found — rotate and move to environment variables`));
    hasRec = true;
  }
  if (report.tornCode > 0) {
    console.log('  ' + c.amber(pad('fix', 10)) + '  ' + c.amber(`${report.tornCode} unimplemented function(s) — audit before any production deploy`));
    hasRec = true;
  }
  if (report.aiArtifacts > 0) {
    console.log('  ' + c.purple(pad('review', 10)) + '  ' + c.purple(`${report.aiArtifacts} AI-generated pattern(s) — manually verify each one is correct`));
    hasRec = true;
  }
  if (report.errorSwallow > 0) {
    console.log('  ' + c.amber(pad('fix', 10)) + '  ' + c.amber(`${report.errorSwallow} empty catch block(s) — add logging at minimum`));
    hasRec = true;
  }
  if (!hasRec) {
    console.log('  ' + c.green(pad('clean', 10)) + '  ' + c.green('No critical recommendations — project is in good health'));
  }

  // Footer
  console.log('');
  console.log('  ' + c.faint('sork scan --file <path>') + c.faint('  ·  deep scan a specific file'));
  console.log('  ' + c.faint('sork guard') + c.faint('               ·  watch for issues in real time'));
  console.log('  ' + c.faint('sork review --staged') + c.faint('     ·  review staged files before commit'));
  console.log('');
}
