...
function shouldWatch(p: string): boolean {
  return (
    WATCHABLE.has(path.extname(p).toLowerCase()) && !p.split(path.sep).some((s) => IGNORE.has(s))
  );
}
...
export async function guardProject(projectPath: string): Promise<void> {
  ...
  if (sourceFiles.length > 20) {
    console.log(
      c.faint(`  ... ${sourceFiles.length - 20} more files  ·  sork scan for full report`)
    );
  }
