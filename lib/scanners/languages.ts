...
export type Language =
  ...
  | 'go' | 'java' | 'c' | 'cpp' | 'ruby' | 'php' | 'unknown';
...
export interface LangPattern {
  ...
  category: 'injection' | 'secrets' | 'nullsafe' | 'crypto' | 'auth' | 'logic' | 'stability' | 'debug';
  message: string;
  fixHint: string;
  cwe?: string;
}
...
const TS_PATTERNS: LangPattern[] = [
  {
    id: 'TS001', name: 'SQL Injection', cwe: 'CWE-89',
  ...
  {
    id: 'TS002', name: 'XSS via innerHTML', cwe: 'CWE-79',
  ...
  {
    id: 'TS003', name: 'Hardcoded Secret', cwe: 'CWE-798',
    regex: /(api[_-]?key|secret|password|token|private[_-]?key)\s*[:=]\s*["'`][A-Za-z0-9+/=_\-]{8,}/i,
  ...
  {
    id: 'TS004', name: 'Unsafe eval()', cwe: 'CWE-95',
  ...
  {
    id: 'TS007', name: 'Command Injection', cwe: 'CWE-78',
  ...
  {
    id: 'TS008', name: 'Prototype Pollution', cwe: 'CWE-1321',
  ...
  {
    id: 'TS009', name: 'Insecure Random', cwe: 'CWE-338',
...
const PY_PATTERNS: LangPattern[] = [
  {
    id: 'PY001', name: 'SQL Injection', cwe: 'CWE-89',
  ...
  {
    id: 'PY002', name: 'Hardcoded Secret', cwe: 'CWE-798',
  ...
  {
    id: 'PY003', name: 'Command Injection', cwe: 'CWE-78',
  ...
  {
    id: 'PY004', name: 'Unsafe Pickle', cwe: 'CWE-502',
  ...
  {
    id: 'PY005', name: 'Path Traversal', cwe: 'CWE-22',
    regex: /open\s*\([^)]*\+|open\s*\(f["'][^)]*\{/,
    severity: 'HIGH', category: 'injection',
    message: 'File open with dynamic path may allow path traversal.',
    fixHint: 'Use pathlib.Path(base_dir, user_input).resolve() and validate it stays within base_dir.',
  },
  {
    id: 'PY006', name: 'Assert in Production', cwe: 'CWE-617',
  ...
  {
    id: 'PY007', name: 'Eval Injection', cwe: 'CWE-95',
  ...
  {
    id: 'PY008', name: 'Bare Except', cwe: 'CWE-390',
    regex: /except\s*:/,
    severity: 'MEDIUM', category: 'stability',
  ...
  {
    id: 'PY009', name: 'Debug Print Leak', cwe: 'CWE-117',
  ...
  {
    id: 'PY010', name: 'Mutable Default Arg', cwe: 'CWE-1188',
    regex: /def\s+\w+\([^)]*=\s*(\[\]|\{\}|\(\))/,
...
const RUST_PATTERNS: LangPattern[] = [
  {
    id: 'RS001', name: 'Unsafe Block', cwe: 'CWE-119',
    regex: /\bunsafe\s*\{/,
  ...
  {
    id: 'RS002', name: 'unwrap() on Result/Option', cwe: 'CWE-248',
    regex: /\.unwrap\s*\(\s*\)/,
    severity: 'HIGH', category: 'nullsafe',
  ...
  {
    id: 'RS003', name: 'expect() without context', cwe: 'CWE-248',
  ...
  {
    id: 'RS004', name: 'Hardcoded Secret', cwe: 'CWE-798',
    regex: /(api_key|secret|password|token)\s*=\s*["'][A-Za-z0-9+/=_\-]{8,}/i,
    severity: 'CRITICAL', category: 'secrets',
  ...
  {
    id: 'RS005', name: 'SQL Injection (sqlx raw)', cwe: 'CWE-89',
    regex: /sqlx::query\s*\(\s*&?format!\s*\(/,
    severity: 'CRITICAL', category: 'injection',
    message: 'SQL query built with format!() macro — injection risk.',
    fixHint: 'Use sqlx::query!() macro with bound parameters: query!("SELECT ... WHERE id = ?", id)',
  },
  {
    id: 'RS006', name: 'Integer Overflow Risk', cwe: 'CWE-190',
...
const GO_PATTERNS: LangPattern[] = [
  {
    id: 'GO001', name: 'SQL Injection', cwe: 'CWE-89',
  ...
  {
    id: 'GO002', name: 'Error Ignored', cwe: 'CWE-390',
  ...
  {
    id: 'GO003', name: 'Hardcoded Secret', cwe: 'CWE-798',
  ...
  {
    id: 'GO004', name: 'Goroutine Leak Risk', cwe: 'CWE-404',
  ...
  {
    id: 'GO005', name: 'Race Condition (map)', cwe: 'CWE-362',
    regex: /map\[.*\]\s*\{[^}]*\}(?!.*sync\.)/,
    severity: 'MEDIUM', category: 'logic',
...
const JAVA_PATTERNS: LangPattern[] = [
  {
    id: 'JV001', name: 'SQL Injection', cwe: 'CWE-89',
    regex: /Statement\.execute\s*\(|createStatement\s*\(\s*\)[^;]*\+/,
    severity: 'CRITICAL', category: 'injection',
    message: 'SQL built with string concatenation — use PreparedStatement.',
    fixHint: 'PreparedStatement ps = conn.prepareStatement("SELECT ... WHERE id = ?"); ps.setInt(1, id);',
  },
  {
    id: 'JV002', name: 'NullPointerException Risk', cwe: 'CWE-476',
    regex: /\w+\.\w+\s*\(\s*\)\.(\w+)/,
    severity: 'MEDIUM', category: 'nullsafe',
  ...
  {
    id: 'JV003', name: 'Hardcoded Secret', cwe: 'CWE-798',
    regex: /(apiKey|secret|password|token)\s*=\s*["'][A-Za-z0-9+/=_\-]{8,}/i,
    severity: 'CRITICAL', category: 'secrets',
  ...
  {
    id: 'JV004', name: 'Unsafe Deserialization', cwe: 'CWE-502',
...
const UNIVERSAL_PATTERNS: LangPattern[] = [
  {
    id: 'UN001', name: 'TODO in Production Code', cwe: undefined,
  ...
  {
    id: 'UN002', name: 'Placeholder / Stub Function', cwe: undefined,
    regex: /throw\s+new\s+Error\s*\(\s*["']not\s+implemented|raise\s+NotImplementedError|panic!\s*\(\s*["']not\s+implemented/i,
    severity: 'HIGH', category: 'stability',
  ...
  {
    id: 'UN003', name: 'Empty Catch Block', cwe: 'CWE-390',
  ...
  {
    id: 'UN004', name: 'AWS/GCP/Azure Key Pattern', cwe: 'CWE-798',
  ...
  {
    id: 'UN005', name: 'Private Key Block', cwe: 'CWE-321',
    regex: /-----BEGIN\s+(RSA|EC|OPENSSH|DSA|PGP)\s+PRIVATE\s+KEY/,
    severity: 'CRITICAL', category: 'secrets',
    message: 'Private key embedded in source — critical credential exposure.',
    fixHint: 'Remove immediately, rotate the key, use a secrets manager (Vault, AWS Secrets Manager).',
  },
];
